    ...
    async handleSubmit(evt) {
      ...
      try {
        ...
        if (theme.settings.afterAtc === 'page') {
          ...
          const giftWrapInput = document.querySelector('input[name="gift_wrap"]');
          if (giftWrapInput && giftWrapInput.checked) {
            const variantId = giftWrapInput.value; // Assume value is the variant ID
  
            // Add gift wrap to cart
            const giftWrapResponse = await fetch("/cart/add.js", {
              method: "POST",
              headers: {
            ...
            if (!giftWrapResponse.ok) {
              throw new Error("Failed to add gift wrap to cart.");
            }
  
            const giftWrapData = await giftWrapResponse.json();
